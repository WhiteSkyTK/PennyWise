...
interface CategoryLimitDao {
    ...
    @Query("SELECT * FROM category_limits WHERE month = :month AND category = :category LIMIT 1")
    suspend fun getCategoryLimitForMonth(category: String, month: String): CategoryLimit?
